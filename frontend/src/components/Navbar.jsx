        ...
        <div className="flex justify-between items-center h-20">
                  ...
                  <div key={idx} className="group flex flex-col items-center">
                    <Link
                      ...
                      className={`relative px-4 py-1 rounded-2xl font-['Poppins'] transition-colors duration-300
                                 ${isActive ? 'text-red-500' : 'text-gray-900 dark:text-white hover:text-red-500'}`}
                    >
                      {item.name}
                    </Link>
                    <div 
                      className={`h-0.5 mt-1 transition-all duration-300 ${isActive ? 'w-full' : 'w-0 group-hover:w-full'} bg-red-500`}
                    ></div>
                  </div>
                );
              })}
          ...
          <div className="flex items-center space-x-4 ml-auto">
              {!user ? (
                <Link
                  to="/login"
                  className="flex items-center gap-2 px-4 py-2 bg-gradient-to-b from-red-600 to-red-700 
      ...
      >
        {menuItems.map((item, idx) => (
          <Link
            ...
            className="relative text-gray-900 dark:text-white text-lg font-semibold
                 ...
                 after:h-[2px] after:bg-[#dc2626] after:transition-all after:duration-500 
                 hover:after:w-full hover:text-[#dc2626] dark:hover:text-[#dc2626]"
          >
            {item.name}
          </Link>
        ))}
        ...
        {!user ? (
          <Link
            to="/login"
            onClick={() => setIsOpen(false)}
            className="block px-4 py-2 text-lg bg-[#dc2626] text-white rounded hover:bg-[#dc2626] dark:hover:bg-[#dc2626] transition-all duration-200"
          >
            Login
          </Link>
        ) : (
          <button
            onClick={handleLogout}
            className="block w-full text-left px-4 py-2 text-lg bg-[#dc2626] text-white rounded hover:bg-[#dc2626] dark:hover:bg-[#dc2626] transition-all duration-200"
          >
            Logout
          </button>
        )}
