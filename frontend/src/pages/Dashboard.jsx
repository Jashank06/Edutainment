  ...
  const BadgesModal = ({ badges, onClose }) => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative z-10 w-full max-w-3xl mx-4 bg-gray-900 border border-[#dc2626]/40 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold text-[#dc2626]">My Badges</h3>
            <button onClick={onClose} className="px-3 py-1 rounded bg-[#dc2626]/20 border border-[#dc2626]/40 hover:bg-[#dc2626]/30">Close</button>
          </div>
          {(!badges || badges.length === 0) ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {badges.map((badge, idx) => (
                <div key={idx} className="bg-black/40 border border-[#dc2626]/30 rounded-xl p-3 flex flex-col items-center gap-2">
                  <img
                    src={`/Badge-${badge.badgeType}.${(badge.badgeType === '50') ? 'jpeg' : 'jpg'}`}
                    alt={`${badge.badgeType}% Badge`}
